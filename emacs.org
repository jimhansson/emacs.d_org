...
*** When pulling between my work machine and private machine I get dos line-endings 
		on every line, this is not good, my work machine will not like it and fail to tangle
		this file.
...
*** Server																																	 :keybindings:
      ...
      (global-set-key (kbd "C-M-x C-M-c") 'save-buffers-kill-emacs)
		...
		My fingers are hardwired to type C-x k for killing buffers so most of the time a
		abourt that action and type C-x C-c after that. but maybe this can solve it in a nice
		way.
...
***** SQL-mode																																		 :hooks:
			...
			#+BEGIN_SRC emacs-lisp
        (defun jiha/comint-write-history-on-exit (process event)
          (message "saving input ring")
        ...
        (defun jiha/turn-on-comint-history (filename)
          (let ((process (get-buffer-process (current-buffer))))
            (when process
              (setq comint-input-ring-file-name filename)
              (comint-read-input-ring)
              (set-process-sentinel process #'jiha/comint-write-history-on-exit))))
        ...
        (add-hook 'sql-interactive-mode-hook
                  (lambda ()
                    (jiha/turn-on-comint-history (concat
                                                  (getenv "TMP")
                                                  "\\"
                                                  sql-connection
                                                  ".history"))))
      ...
      #+RESULTS:
	    | lambda | nil | (jiha/turn-on-comint-history (concat (getenv TMP) \ sql-connection .history)) |
	    | lambda | nil | (toggle-truncate-lines 1)                                                     |
			
			but that is not enough, when killing the buffer without exit, the history will not
			be saved. by adding comint-write-input-ring to the kill-buffer hook for all comint
			derived modes we fix that

			#+BEGIN_SRC emacs-lisp
        (add-hook 'kill-buffer-hook 'comint-write-input-ring)
			#+END_SRC

			when killing emacs the kill-buffer-hook's will not be run for some reason, to fix
			that we add a looping over all buffers in kill-emacs-hook

			#+BEGIN_SRC emacs-lisp
        (defun jiha/mapc-buffers (fn)
          (mapc (lambda (buffer)
                  (with-current-buffer buffer
                    (funcall fn)))
                (buffer-list)))

        (defun jiha/comint-write-input-ring-all-buffers ()
          (jiha/mapc-buffers 'comint-write-input-ring))

        (add-hook 'kill-emacs-hook 'jiha/comint-write-input-ring-all-buffers)
...
******* Support Multiple connections
        	...
        	(add-hook 'sql-interactive-mode-hook 'sql-rename-buffer)
				...
				#+RESULTS:
				| sql-rename-buffer | (lambda nil (rename-buffer (sql-make-alternate-buffer-name))) | (lambda nil (jiha/turn-on-comint-history (concat (getenv TMP) \ sql-connection .history))) | (lambda nil (toggle-truncate-lines 1)) |
