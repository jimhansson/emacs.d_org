...
* Iswitch Buffers                                                   :buffers:
  ...
  #+begin_example emacs-lisp
    ...
    ; howto define a face
    (defface jiha-ibuffer-readonly-face-example '((t (:foreground "Red" :weight bold)))
      "Ibuffer readonly face")
    
    ; these are the the standard faces used but with new names so we can
    ; change them in a simple way.
    (defface jiha-ibuffer-readonly-face 'font-lock-constant-face)
    (defface jiha-ibuffer-compressed-file-name-regex-face 'font-lock-doc-face)
    (defface jiha-ibuffer-dired-mode-face 'font-lock-function-name-face)
    (defface jiha-ibuffer-help-modes-face 'font-lock-comment-face)
    (defface jiha-ibuffer-some-weird-face 'italic)
...
*** TODO make sure it works on windows with svg or else change to png's
    :LOGBOOK:
